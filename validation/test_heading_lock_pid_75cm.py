...
def draw_visualization(
    ...
    text_lines = [
        ...
        ("Distance", f"{distance_travelled_cm:.2f} cm"),
...
def log_terminal(
    ...
    print("Distance Travelled:", format_display(distance_travelled_cm, " cm"))
...
def calculate_steps_per_cm(wheel_diameter_cm: float, steps_per_rev: int, microsteps: int) -> float:
    """Return commanded step-pulses per centimetre of wheel travel."""
    wheel_circumference_cm = math.pi * wheel_diameter_cm
    return (steps_per_rev * microsteps) / wheel_circumference_cm


def integrate_commanded_distance_cm(
    previous_distance_cm: float,
    pid_result: PIDResult | None,
    delta_time_sec: float,
    steps_per_cm: float,
) -> float:
    """Estimate travel distance from commanded step frequency.

    This assumes the ESP32/T60 driver follows the requested STEP frequency and
    the wheels do not slip. Replace this with encoder distance later if you add
    feedback.
    """
    if pid_result is None or delta_time_sec <= 0.0 or steps_per_cm <= 0.0:
        return previous_distance_cm

    average_frequency_hz = (
        abs(pid_result.left_frequency_hz) + abs(pid_result.right_frequency_hz)
    ) / 2.0
    return previous_distance_cm + (average_frequency_hz * delta_time_sec / steps_per_cm)
...
def run_validation(args: argparse.Namespace) -> None:
    ...
    last_distance_update_time: float | None = None
    steps_per_cm = calculate_steps_per_cm(
    ...
    try:
        while True:
            ...
            if moving_started and state == "HEADING_HOLD":
                if last_distance_update_time is None:
                    last_distance_update_time = now
                else:
                    distance_travelled_cm = integrate_commanded_distance_cm(
                        distance_travelled_cm,
                        latest_pid_result,
                        now - last_distance_update_time,
                        steps_per_cm,
                    )
                    last_distance_update_time = now
            ...
            if state == "WAIT_FOR_TAG1" and measurement.tag_id == 1:
                ...
                if state == "FORWARD_START_DELAY":
                    ...
                    if distance_travelled_cm >= args.travel_distance_cm:
                        print("## 75 CM HEADING VALIDATION COMPLETE")
                        print("Distance Travelled:", format_display(distance_travelled_cm, " cm"))
                        current_command = "STOP"
                        transmit_command(
                            serial_controller,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--travel-distance-cm",
        type=float,
        default=TRAVEL_DISTANCE_CM,
        help="Validation travel distance before automatic STOP.",
    )
    parser.add_argument(
        "--wheel-diameter-cm",
        type=float,
        default=DEFAULT_WHEEL_DIAMETER_CM,
        help="Wheel diameter used for commanded-distance estimation.",
    )
    parser.add_argument(
        "--steps-per-rev",
        type=int,
        default=DEFAULT_STEPS_PER_REV,
        help="Full motor steps per wheel revolution before microstepping.",
    )
    parser.add_argument(
        "--microsteps",
        type=int,
        default=DEFAULT_MICROSTEPS,
        help="T60 driver microstep setting.",
    )
